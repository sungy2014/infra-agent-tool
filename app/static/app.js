...
function handleEvent(jobId, ev) {
  ...
  if (ev.type === 'message') {
    ...
    view.insertAdjacentHTML('beforeend',
      `<div class="conv-message conv-tool" style="border-color:${statusColor}">
        <div class="conv-role">Jenkins build</div>
        <div class="conv-body">${icon} Build #${escHtml(d.build_number || '?')}: <strong>${escHtml(d.result || 'UNKNOWN')}</strong>${linkHtml}</div>${consoleText}
      </div>`
    );
    view.scrollTop = view.scrollHeight;
